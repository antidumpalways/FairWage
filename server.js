...
const FAIRWAGE_WASM_HASH = 'bcadbb1b80a30c4d46e4b512d6586a028f45b77afe47557c46c49621fb485bc7';
...
function setCurrentContractId(contractId, type) {
  ...
  if (type === 'fairwage') {
    ...
    console.log('ℹ️ Unknown contract type. Not setting current contract automatically.');
  }
}
...
app.post('/api/set-contract-id', (req, res) => {
  try {
    ...
    if (!contractId || !type) {
      return res.status(400).json({ success: false, error: 'Missing contractId or type' });
    }
    setCurrentContractId(contractId, type);
    res.json({ success: true, contractId, type, message: 'Contract ID set successfully' });
  } catch (error) {
...
app.get('/api/get-current-contract', (req, res) => {
  try {
    ...
    res.json({ success: false, error: error.message, message: 'No contract ID available' });
  }
});
...
app.post('/api/prepare-token-deploy', async (req, res) => {
  try {
    ...
    console.log('🚀 Preparing REAL token deployment for:', userPublicKey, 'Name:', tokenName, 'Symbol:', tokenSymbol);

    if (!userPublicKey || !tokenName || !tokenSymbol) {
      return res.status(400).json({ error: 'Missing required fields: userPublicKey, tokenName, tokenSymbol' });
    }
    ...
    res.status(500).json({ success: false, error: error.message, details: 'Soroban deployment error' });
  }
});
...
app.post('/api/prepare-fairwage-deploy', async (req, res) => {
  try {
    ...
    console.log('🏢 Preparing FairWage DEPLOYMENT for:', userPublicKey, 'Token:', tokenContractId, 'Company:', companyName);

    if (!userPublicKey || !tokenContractId || !companyName) {
      return res.status(400).json({ error: 'Missing required fields: userPublicKey, tokenContractId, companyName' });
    }
...
app.post('/api/prepare-fairwage-initialize', async (req, res) => {
  try {
    ...
    if (!userPublicKey || !fairWageContractId || !tokenContractId || !companyName) {
      return res.status(400).json({ error: 'Missing required fields: userPublicKey, fairWageContractId, tokenContractId, companyName' });
    }
...
app.post('/api/initialize-contract', async (req, res) => {
  try {
    ...
    console.log('🔧 Initializing contract:', { contractId, contractType, userPublicKey, tokenContractId, companyName });

    if (!contractId || !contractType || !userPublicKey) {
      return res.status(400).json({ error: 'Missing required parameters: contractId, contractType, userPublicKey' });
    }
    ...
    if (contractType !== 'fairwage') {
      return res.status(400).json({ error: 'Invalid contract type. Must be "token" or "fairwage"' });
    }
    if (!tokenContractId) {
      return res.status(400).json({ error: 'tokenContractId required for FairWage initialization' });
    }
...
app.post('/api/submit-transaction', async (req, res) => {
  try {
    ...
    if (transaction.signatures.length === 0) {
      return res.status(400).json({ error: 'Transaction not signed! Please sign with your wallet first.' });
    }

    const sendResult = await server.sendTransaction(transaction);
    console.log('✅ Transaction submitted:', sendResult.hash);
    console.log('🔗 Verify at: https://stellar.expert/explorer/testnet/tx/' + sendResult.hash);

    // Poll confirmation
    let getResponse = await server.getTransaction(sendResult.hash);
    let attempts = 0;
    while ((getResponse.status === 'NOT_FOUND' || getResponse.status === 'PENDING') && attempts < 60) {
    ...
    if (contractId && (contractType === 'fairwage' || contractType === 'token')) {
...
app.get('/api/contract-info', async (req, res) => {
  try {
    ...
    if (!contractId) return res.status(400).json({ error: 'Missing contractId parameter' });
    ...
    let parsed = { contractId, latestLedger: contractInfo.latestLedger, entries: [] };
    ...
    res.json({ success: true, contractId, contractInfo: parsed, message: 'Contract info retrieved and parsed successfully' });
  } catch (error) {
...
app.post('/api/add-employee', async (req, res) => {
  try {
    ...
    console.log('👤 Adding employee:', { userPublicKey, fairWageContractId, employeeAddress, employeeName, wageRate, wagePeriod });

    if (!userPublicKey || !fairWageContractId || !employeeAddress || !employeeName || !wageRate) {
      return res.status(400).json({ error: 'Missing required parameters: userPublicKey, fairWageContractId, employeeAddress, employeeName, wageRate' });
    }
    ...
    const tx = new StellarSdk.TransactionBuilder(sourceAccount, { fee: '100000', networkPassphrase })
      .addOperation(op)
      .setTimeout(30)
      .build();
...
app.post('/api/set-salary', async (req, res) => {
  try {
    ...
    if (!userPublicKey || !fairWageContractId || !employeeAddress || !wageRate) {
      return res.status(400).json({ error: 'Missing required parameters: userPublicKey, fairWageContractId, employeeAddress, wageRate' });
    }
...
app.post('/api/update-wage-rate', async (req, res) => {
  try {
    ...
    if (!userPublicKey || !fairWageContractId || !employeeAddress || !newWageRate) {
      return res.status(400).json({ success: false, error: 'Missing required parameters: userPublicKey, fairWageContractId, employeeAddress, newWageRate' });
    }
    if (newWageRate <= 0) {
      return res.status(400).json({ success: false, error: 'Wage rate must be greater than 0' });
    }
...
app.post('/api/pay-employee', async (req, res) => {
  try {
    ...
    if (!userPublicKey || !fairWageContractId || !employeeAddress || !amount) {
      return res.status(400).json({ error: 'Missing required parameters: userPublicKey, fairWageContractId, employeeAddress, amount' });
    }
...
app.post('/api/remove-employee', async (req, res) => {
  try {
    ...
    if (!userPublicKey || !fairWageContractId || !employeeAddress) {
      return res.status(400).json({ error: 'Missing required parameters: userPublicKey, fairWageContractId, employeeAddress' });
    }
...
app.post('/api/fund-contract', async (req, res) => {
  try {
    ...
    if (!userPublicKey || !fairWageContractId || !tokenContractId || !amount) {
      return res.status(400).json({ success: false, error: 'Missing required parameters: userPublicKey, fairWageContractId, tokenContractId, amount' });
    }
...
app.post('/api/check-contract-balance', async (req, res) => {
  try {
    ...
    if (!fairWageContractId || !tokenContractId) {
      return res.status(400).json({ success: false, error: 'Missing required parameters: fairWageContractId, tokenContractId' });
    }
    ...
    if (simulation.error) throw new Error(`Simulation failed: ${simulation.error.message}`);
    ...
    res.json({ success: true, balance, fairWageContractId, tokenContractId, message: 'Contract balance retrieved successfully' });
  } catch (error) {
...
app.post('/api/create-trustline', async (req, res) => {
  try {
    ...
    if (!userPublicKey || !tokenContractId || !employeeAddress) {
      return res.status(400).json({ success: false, error: 'Missing required parameters: userPublicKey, tokenContractId, employeeAddress' });
    }
...
app.post('/api/pay-partial-employer', async (req, res) => {
  try {
    ...
    res.json({ success: true, transactionXdr: preparedTx.toXDR(), message: 'Pay partial by employer transaction prepared successfully' });
  } catch (error) {
    console.error('❌ Error paying partial by employer:', error);
    res.status(500).json({ success: false, error: 'Failed to pay partial by employer', details: error.message });
  }
});
...
app.post('/api/payday-sweep-many', async (req, res) => {
  try {
    ...
    if (!userPublicKey || !fairWageContractId || !employeeAddresses || !Array.isArray(employeeAddresses)) {
      return res.status(400).json({ error: 'Missing required parameters: userPublicKey, fairWageContractId, employeeAddresses (array)' });
    }

    const sourceAccount = await horizonServer.loadAccount(userPublicKey);

    const addressArray = employeeAddresses.map((addr) => StellarSdk.Address.fromString(addr).toScVal());
    ...
    res.json({ success: true, transactionXdr: preparedTx.toXDR(), message: 'Payday sweep many transaction prepared successfully' });
  } catch (error) {
    console.error('❌ Error paying all employees:', error);
    res.status(500).json({ success: false, error: 'Failed to pay all employees', details: error.message });
  }
});
...
app.post('/api/accrued-balance', async (req, res) => {
  try {
    ...
    if (!employeeAddress) return res.status(400).json({ error: 'Missing required parameters: employeeAddress' });

    const useContractId = contractId || getFairWageContractId();
    if (!useContractId) return res.status(400).json({ error: 'FairWage contract not configured' });
    ...
    res.json({ success: true, balance, message: 'Accrued balance retrieved successfully' });
  } catch (error) {
    console.error('❌ Error getting accrued balance:', error);
    res.status(500).json({ success: false, error: 'Failed to get accrued balance', details: error.message });
  }
});
...
app.post('/api/get-accrued-balance', async (req, res) => {
  try {
    ...
    if (!fairWageContractId || !employeeAddress) {
      return res.status(400).json({ error: 'Missing required parameters: fairWageContractId, employeeAddress' });
    }
  ...
  try {
    ...
    if (simulation.result && simulation.result.retval) {
      try {
        ...
        employeeInfo = {
          wage_rate: native.wage_rate?.toString?.() ?? String(native.wage_rate || '0'),
          accrued_balance: native.accrued_balance?.toString?.() ?? String(native.accrued_balance || '0'),
          last_accrual_timestamp: Number(
            (native.last_accrual_timestamp?.toString?.() ?? native.last_accrual_timestamp) || 0
          ),
    ...
    res.json({ success: true, employeeInfo, message: 'Employee info retrieved successfully' });
  } catch (error) {
    console.error('❌ Error getting employee info:', error);
    res.status(500).json({ success: false, error: 'Failed to get employee info', details: error.message });
  }
});
...
app.post('/api/get-contract-balance', async (req, res) => {
  try {
    ...
    if (!fairWageContractId) return res.status(400).json({ error: 'Missing required parameters: fairWageContractId' });
    ...
    res.json({ success: true, balance, message: 'Contract balance retrieved successfully' });
  } catch (error) {
    console.error('❌ Error getting contract balance:', error);
    res.status(500).json({ success: false, error: 'Failed to get contract balance', details: error.message });
  }
});
...
app.post('/api/list-employees', async (req, res) => {
  try {
    ...
    res.json({ success: true, employees, message: 'Employees listed successfully' });
  } catch (error) {
    console.error('❌ Error listing employees:', error);
    res.status(500).json({ success: false, error: 'Failed to list employees', details: error.message });
  }
});
...
app.post('/api/freeze-employee', async (req, res) => {
  try {
    ...
    res.json({ success: true, transactionXdr: preparedTx.toXDR(), message: 'Freeze employee transaction prepared successfully' });
  } catch (error) {
    console.error('❌ Error preparing freeze employee transaction:', error);
    res.status(500).json({ success: false, error: 'Failed to prepare freeze employee transaction', details: error.message });
  }
});
...
app.post('/api/activate-employee', async (req, res) => {
  try {
    ...
    res.json({ success: true, transactionXdr: preparedTx.toXDR(), message: 'Activate employee transaction prepared successfully' });
  } catch (error) {
    console.error('❌ Error preparing activate employee transaction:', error);
    res.status(500).json({ success: false, error: 'Failed to prepare activate employee transaction', details: error.message });
  }
});
...
app.post('/api/employee-withdraw', async (req, res) => {
  try {
    ...
    if (!userPublicKey || !fairWageContractId || amount === undefined) {
      return res.status(400).json({ success: false, error: 'Missing required parameters: userPublicKey, fairWageContractId, amount' });
    }
...
// ================================
app.listen(PORT, () => {
  console.log(`🚀 FairWage Backend API Server running on port ${PORT}`);
